...
# Need to patch bug in applyFilter from spatialfil package
applyFilter <- function (x, kernel) 
  {
      if (inherits(kernel, "convKern") == FALSE) 
      ...
      if (inherits(x, "array", which = TRUE) == 1) {
...
makeTeamBPlot <- function(z, dat, ylab, xlab, breaks, strip = FALSE){
  ...
  ggplot(
    mapping = aes(y = Liberals, x = Conservatives, z = eval(parse(text = z)), fill = after_stat(level))
    , data = dat
  ) +
...
otm1_models <- subset(combiThetas, figType == "Model") %>%
  ...
  stat_contour(geom = "polygon", aes(z = Density, fill = after_stat(level))) +
