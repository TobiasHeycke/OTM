...
```{r otm1-power-results}
som_cache <- "manuscript_cache/latex/"
rydell_pes <- knitr::load_cache(label = "rydell-effects", object = "rydell_pes", path = som_cache)
rydell_dz <- knitr::load_cache(label = "rydell-effects", object = "rydell_dz", path = som_cache)
...
```{r power-curve-plot, fig.width = 7, fig.height = 8, fig.cap = "(ref:otm1-power-curve)"}
...
Also in line with the ANOVA analysis, we found that response time differences indicated *more* favorable evaluations after the first than after the second block when the behavioral information was first positive and later negative, `r otm1_iat_lmer_contrasts$full_result$Positive_negative`.
...
#### Directly and indirectly measured evaluations
...
This is because our data are informative enough to overwhelm the priors and because these Bayes factors primarily depend on the shape and location of the joint posterior distribution [@klugkist_bayesian_2005].
