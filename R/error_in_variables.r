...
summary.eiv_lm <- function(x) {
  ...
  p_alpha <- 2 * pnorm(-abs(t_alpha)) # compare to getAnywhere(t.test.default)
  ...
  res <- c(data.frame(term = coef_names), as.data.frame(unclass(x))[6:5, ], list(statistic=c(t_alpha, t_beta), p.value=c(p_alpha, p_beta)))
...
apa_print.eiv_lm <- function(
  ...
  regression_table$term <- papaja:::beautify_terms(regression_table$term, standardized = FALSE)
  ...
  tinylabels::variable_label(regression_table) <- c(
    term = "Predictor"
  ...
  apa_res <- papaja::glue_apa_results(
    regression_table
    , est_glue = papaja:::construct_glue(regression_table, "estimate")
    , stat_glue = papaja:::construct_glue(regression_table, "statistic")
    , in_paren = in_paren
    , est_first = TRUE
    , simplify = FALSE
  )
